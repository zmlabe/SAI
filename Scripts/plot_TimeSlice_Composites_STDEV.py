...
and ARISE for standard deviation
...
if typeOfSlice == 'direct':
    timeslice = 10
    beforeSAI = np.nanstd(arisem[:,injectionyear-timeslice:injectionyear,:,:],axis=1)
    afterSAI = np.nanstd(arisem[:,injectionyear:injectionyear+timeslice,:,:],axis=1)
    beforeCON = np.nanstd(waccmm[:,injectionyear-timeslice:injectionyear,:,:],axis=1)
    afterCON = np.nanstd(waccmm[:,injectionyear:injectionyear+timeslice,:,:],axis=1)
elif typeOfSlice == 'later':
    timeslice = 10
    futurelater = 20
    beforeSAI = np.nanstd(arisem[:,injectionyear-timeslice:injectionyear,:,:],axis=1)
    afterSAI = np.nanstd(arisem[:,injectionyear+futurelater:injectionyear+timeslice+futurelater,:,:],axis=1)
    beforeCON = np.nanstd(waccmm[:,injectionyear-timeslice:injectionyear,:,:],axis=1)
    afterCON = np.nanstd(waccmm[:,injectionyear+futurelater:injectionyear+timeslice+futurelater,:,:],axis=1)
...
limitd = np.arange(-0.5,0.51,0.01)
barlimd = np.round(np.arange(-0.5,0.6,0.5),2)
limitc = np.arange(0,1.01,0.01)
barlimc = np.round(np.arange(0,1.1,0.5),2)
...
for mo in range(len(monthlychoiceq)):                                                                                                                         
    label = r'\textbf{std.dev. - %s -- %s -- [mm/day]}' % (monthlychoiceq[mo],typeOfSlice)
    ...
    plt.tight_layout()
    # plt.subplots_adjust(top=0.85,wspace=0.02,hspace=0.05,bottom=0.14)
    
    plt.savefig(directoryfigure + 'SAI_Composites_PRECT_STDEV_%s_%s.png' % (monthlychoiceq[mo],typeOfSlice),dpi=300)
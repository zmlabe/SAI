"""
Function(s) reads in monthly data from WACCM for different variables using # of
ensemble members
...
Usage
-----
    [1] read_WACCM(directory,vari,sliceperiod,
                  slicebase,sliceshape,addclimo,
                  slicenan,takeEnsMean)
"""

def read_WACCM(directory,vari,sliceperiod,slicebase,sliceshape,addclimo,slicenan,takeEnsMean):
    """
    Function reads monthly data from WACCM
    ...
    Usage
    -----
    read_WACCM(directory,vari,sliceperiod,
                  slicebase,sliceshape,addclimo,
                  slicenan,takeEnsMean,timeper)
    """
    print('\n>>>>>>>>>> STARTING read_WACCM function!')
    ...
    for i,ensmember in enumerate(ens):
        if int(ensmember) < 6:
            filename = directory + '%s/%s_WACCM_%s_2015-2100.nc' % (vari,vari,ensmember)
            ...
            data.close()
        if int(ensmember) > 5:
            filename = directory + '%s/%s_WACCM_%s_2015-2069.nc' % (vari,vari,ensmember)
            data = Dataset(filename,'r')
            lat1 = data.variables['latitude'][:]
            lon1 = data.variables['longitude'][:]
            var = data.variables['%s' % vari][:sizeOfTime,:,:]
        ...
        print('Completed: read WACCM ensemble --%s--' % ensmember)

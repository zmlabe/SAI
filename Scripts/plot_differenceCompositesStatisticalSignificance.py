...
for i in range(len(monthlychoiceq)):
    pruns[i,:,:] = UT.calc_FDR_ttest(diffCONrun[i],diffSAIrun[i],alpha_f)
...
for mo in range(len(monthlychoiceq)):                                                                                                                         
    ...
    var = np.nanmean(diffCONrun[mo] - diffSAIrun[mo],axis=0)
    pvar = pruns[mo]
    ...
    plt.savefig(directoryfigure + 'CompositeStatisticalSignificances_PRECT_%s_%s.png' % (monthlychoiceq[mo],typeOfSlice),dpi=300)
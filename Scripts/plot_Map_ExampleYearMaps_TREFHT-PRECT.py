...
waccmt = datat_waccm[ensindext,yearindex,:,:]
waccmp = datap_waccm[ensindexp,yearindex,:,:]
ariset = datat_arise[ensindext,yearindex,:,:]
arisep = datap_arise[ensindexp,yearindex,:,:]
...
plotvar = [ariset,waccmt,
           arisep,waccmp]
colormaps = [plt.cm.twilight,plt.cm.twilight,
             cmr.dusk,cmr.dusk]
limits = [np.arange(-35,36,1),np.arange(-35,36,1),
          np.arange(0,10.1,0.25),np.arange(0,10.1,0.25)]
barlims = [np.round(np.arange(-35,36,10),2),np.round(np.arange(-35,36,10),2),
          np.round(np.arange(0,11,2),2),np.round(np.arange(0,11,2),2)]
...
plt.tight_layout()
plt.savefig(directoryfigure + 'MAP_ExampleYearMaps_TREFHT-PRECT.png',dpi=300)

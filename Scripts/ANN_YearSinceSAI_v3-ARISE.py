...
###############################################################################
    ...
    XAI = get_integrated_gradients(XtestS, baseline=None, num_steps=50, top_pred_idx=None)
    np.savez(directoryoutput + 'integratedGradients.npz',XAI=np.asarray(XAI).reshape(35,96,144))
    sys.exit()
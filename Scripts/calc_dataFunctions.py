...
def readFiles(variq,dataset,monthlychoice,numOfEns,lensalso,randomalso,ravelyearsbinary,ravelbinary,shuffletype,timeper):
    ...
    if dataset == 'BEST':
        ...
        sliceyearER = np.arange(1979,2020+1,1)
        sliceshapeER = 3
        slicenanER = 'nan'
        addclimoER = True
        ENSmean = np.nan
        ...
        sliceyearBE = np.arange(1950,2020+1,1)
        sliceshapeBE = 3
        slicenanBE = 'nan'
        addclimoBE = True
        ENSmean = np.nan
        ...
        ENSmean = np.nan
        addclimoLEL = True
        slicebaseLEL = np.arange(1951,1980+1,1)
        takeEnsMeanLEL = False
        lat1,lon1,data,ENSmean = LEL.read_LENS(directorydataLEL,variq,
                                              monthlychoice,
                                              slicebaseLEL,sliceshapeLEL,
                                              addclimoLEL,slicenanLEL,
                                              takeEnsMeanLEL,timeper) 
    elif dataset == 'WACCM':
        ...
        ENSmeanWAC = np.nan
        addclimoWAC = True
        slicebaseWAC = np.arange(1951,1980+1,1)
        takeEnsMeanWAC = False
        lat1,lon1,data,ENSmeanWAC = WAC.read_WACCM(directorydataWAC,variq,
                                              monthlychoice,
                                              slicebaseWAC,sliceshapeWAC,
                                              addclimoWAC,slicenanWAC,
                                              takeEnsMeanWAC) 
    elif dataset == 'ARISE':
        ...
        ENSmeanARI = np.nan
        addclimoARI = True
        slicebaseARI = np.arange(1951,1980+1,1)
        takeEnsMeanARI = False
        lat1,lon1,data,ENSmeanARI = ARI.read_ARISE(directorydataARI,variq,
                                              monthlychoice,
                                              slicebaseARI,sliceshapeARI,
                                              addclimoARI,slicenanARI,
                                              takeEnsMeanARI) 
        ...
        if any([timeper=='historical',timeper=='all',timeper=='hiatus']):
    ...
    elif dataset == 'all_cesmlens':
        import calc_CESMLargeEnsemblesTogether as CESMall
        directorydataCESMall = '/Users/zlabe/Data/'
        sliceshapeCESMall = 4
        slicenanCESMall = 'nan'
        ENSmean = np.nan
        addclimoCESMall = True
        slicebaseCESMall = np.arange(1951,1980+1,1)
        takeEnsMeanCESMall = False
        lat1,lon1,data,= CESMall.read_ALLCESM_le(directorydataCESMall,variq,
                                              monthlychoice,
...
# shuffletype = 'GAUSS'
# timeper = 'historical'
# data,lat1,lon1 = readFiles('PRECT','ARISE','annual',numOfEns,lensalso,

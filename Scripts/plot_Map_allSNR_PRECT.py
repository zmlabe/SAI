...
### Calculate trends over all years
per1_trend_waccm = calcTrend(data_waccm[:,:,:,:])
per1_trend_arise = calcTrend(data_arise[:,:,:,:])
...
cbar_ax = fig.add_axes([0.34,0.13,0.35,0.026])                
cbar = fig.colorbar(cs1,cax=cbar_ax,orientation='horizontal',
                    extend='max',extendfrac=0.07,drawedges=False)
cbar.set_label(r'\textbf{PRECT [Signal-To-Noise] for 2035-2069}',fontsize=9,color='dimgrey',labelpad=1.4)  
...
plt.savefig(directoryfigure + 'Map_allSNR_PRECT.png',dpi=300)

### Save composite for figure
directoryoutput = '/Users/zlabe/Documents/Research/SolarIntervention/Data/'
np.savez(directoryoutput + 'PRECT-SAI_all.npz',
         snr=snr1_arise,lat=lats,lon=lons)

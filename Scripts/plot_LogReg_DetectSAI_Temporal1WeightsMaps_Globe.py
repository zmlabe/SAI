...
comp_t_con_ready = np.nanmean(weights_t_con[yearq1,:,:],axis=0)
comp_p_sai_ready = np.nanmean(weights_p_sai[yearq1,:,:],axis=0)
comp_p_con_ready = np.nanmean(weights_p_con[yearq1,:,:],axis=0)
...
plt.savefig(directoryfigure + 'LogReg_DetectSAI_Temporal1WeightsMaps_Globe.png',dpi=300)

"""
ANN for evaluating how long since 2035 (injection) only for ARISE 
...
seasons = ['annual']
variq = 'PRECT'
ANNname = 'Hiddens_10x10'
reg_nameq = ['Globe','NH','SH','Arctic','Antarctic','narrowTropics','SEAsia','NorthAfrica','Amazon']
ridge_penaltyq = np.genfromtxt(directoryModels + 'L2_ANN_YearsSinceSAI_%s_corr_%s.txt' % (variq,ANNname),unpack=True)
...
###############################################################################
    ...
    ### Save the output 
    directoryoutput = '/Users/zlabe/Documents/Research/SolarIntervention/Data/YearsSinceSAI/ONLYARISE/'
    ...
    ### Calculate gradients x inputs
    gradient = get_gradients(XtestS, top_pred_idx=None)
    xain = gradient * XtestS
    xainorm = xain/np.nanmax(xain,axis=(-1))[:,np.newaxis]
    xai = xainorm.reshape(len(yearsall),lats.shape[0],lons.shape[0])
    xai[np.where(xai==0.0)]=np.nan
    
    np.savez(directoryoutput + 'InputsxGradients_' + saveData + '.npz',XAI=xai)
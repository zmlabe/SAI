...
directorydata = '/Users/zlabe/Documents/Research/SolarIntervention/Dark_Data/'
...
cont = np.load(directorydata + 'TREFHT-SAI_allContributions.npz')
contribution = cont['cont'][:]
lats = cont['lat'][:]
lons = cont['lon'][:]
snr = np.load(directorydata + 'TREFHT-SAI_allSNR.npz')
snrall = snr['snr']
...
barlim = np.round(np.arange(0,3.1,1),2)

label = r'\textbf{Signal-To-Noise}'
...
plt.savefig(directoryfigure + 'SampleMaps_SNR-Contributions_TREFHT.png',dpi=1000)
